...
function saveKerusakan(){
  ...
  const fotoFiles=document.getElementById('fFotoKerusakan')?.files||[];
let fotoList=[];
for(let i=0;i<fotoFiles.length;i++){fotoList.push(URL.createObjectURL(fotoFiles[i]));}
kerusakanData.unshift({jpl:d.jpl,km:d.km,jenis,severity:sev,dampak,ket,foto:fotoList,tgl,selesai:false});
...
pollEditorSync();
function previewKerusakanFoto(input){
 const p=document.getElementById("previewKerusakanFoto");
 p.innerHTML="";
 [...input.files].forEach(f=>{
  const r=new FileReader();
  r.onload=e=>{const img=document.createElement("img");img.src=e.target.result;p.appendChild(img);};
  r.readAsDataURL(f);
 });
}
